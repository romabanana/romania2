...
func show_division(division) -> void:
	...
	label_province.text = "Province: %s" % ProvinceManager.get_province_name(
		ProvinceManager.get_province_id(division.current_tile)
	)
	label_terrain.text  = "Terrain: %s" % TerrainManager.TERRAIN_DATA[
		TerrainManager.get_terrain(division.current_tile)
	]["name"]
